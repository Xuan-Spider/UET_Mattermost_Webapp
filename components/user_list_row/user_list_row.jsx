...
export default class UserListRow extends React.Component {
    ...
    handleSubmit = (values = this.props.user.id) => {
        ...
        const done = (result) => {
            ...
            if (!error) {
                this.exitToChannel = '/' + this.props.user.username + '/channels/' + data.name;
                console.log("exitToChannel", this.exitToChannel)
                this.handleHide();
            }
        };
        
        this.props.actions.openDirectChannelToUserId(this.props.user.id).then(done);
        return;
    };
